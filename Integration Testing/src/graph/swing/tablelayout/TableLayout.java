package graph.swing.tablelayout;
...
protected Container containter;

protected List<TableRow> listRow;

protected TableLayoutConstraints defaultConstraint;

	public class TableRow{
		private int row;
		private JComponent[] c;
		
		private TableRow(int row, JComponent... c){
			this.row = row;
			this.c = c;
		}
...
public TableLayout (Container c, double size[][])
{
	this.containter = c;
	this.listRow = new ArrayList<>();
	this.defaultConstraint = new TableLayoutConstraints(0, 0, 0, 0, CENTER, CENTER);
...
private static int parseAlign(String token){
	int align = 0;
	if (token.equals("l"))
		align = LEFT;
	...
	public TableRow insertRow(int row, double size, boolean reload,
			String align, JComponent... c){
		if (row == -1)
			row = getNumRow();
		
		String[] al = align == null ? null : align.toLowerCase().split(" ");
		
		insertRow(row, size);
		for (int i = 0; i < c.length; i++){
			if (c[i] == null) continue;
			TableLayoutConstraints tlc = defaultConstraint.clone();
			tlc.row1 = tlc.row2 = row;
			tlc.col1 = tlc.col2 = i;
			if (al != null){
				tlc.hAlign = parseAlign(al[2*i]);
				tlc.vAlign = parseAlign(al[2*i + 1]);
			}
			containter.add(c[i], tlc);
		}
		...
		TableRow r = new TableRow(row, c);
		listRow.add(r);
		return r;
	}
	
	public void deleteRow(TableRow row){
		for (JComponent c: row.c)
			if (c != null)
				containter.remove(c);
		
		listRow.remove(row);
		deleteRow(row.getRow());
		containter.revalidate();
		containter.repaint();
	}
